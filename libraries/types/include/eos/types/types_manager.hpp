...
namespace eos { namespace types {
   ...
   public:
      ...
      public:
         ...
         inline const types_manager& get_types_manager()const { return tm; }

         friend class types_manager;
      ...
      private:
         ...
         table_index(const types_manager& tm, type_id::index_t index, uint8_t index_seq_num);
      };
      ...
      // Type traversal: (Caller needs to ensure types_manager is in a coherent state, or else things will go horribly wrong)
   ...
   private:
      ...
      types_manager() {}
      types_manager(vector<uint32_t>&& types, vector<field_metadata>&& members, flat_map<string, type_id::index_t> table_lookup) 
         : types(types), members(members), table_lookup(table_lookup) 
      {
      }
      ...
      void             complete_struct(type_id::index_t index, type_id::size_align sa,
                                       uint32_t members_offset, uint16_t total_num_members, uint16_t num_sorted_members,
                                       type_id base = type_id(), field_metadata::sort_order base_so = field_metadata::unsorted);
