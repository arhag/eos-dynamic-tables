...
namespace eos { namespace types {
   ...
   struct bit_view
   {
      ...
      static_assert( is_integral<S>::value && !is_signed<S>::value, "S must be an unsigned integral type" );
      ...
      static_assert( size != 1 || !is_signed<T>::value, "T cannot be signed if size == 1" ); 
      ...
      static inline 
      typename enable_if<!is_signed<U>::value, U>::type
      get( S s ) // Expects T to be unsigned
      {
         static_assert( is_same<U, T>::value, "Do not override default template argument for get method" );
      ...
      template<typename U = T>
      static inline 
      typename enable_if<is_signed<U>::value && size == sizeof(U)*8, U>::type
