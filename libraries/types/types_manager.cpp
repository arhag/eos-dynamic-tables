...
namespace eos { namespace types {
   ...
   types_manager::get_members_common(type_id::index_t struct_index)const
   {
   ...
   types_manager::get_all_members(type_id::index_t struct_index)const
   {
   ...
   range<typename vector<field_metadata>::const_iterator> 
   types_manager::get_sorted_members(type_id::index_t struct_index)const
   {
      uint32_t member_data_offset;
      uint32_t num_sorted_members;
   ...
   field_metadata types_manager::get_member(type_id::index_t struct_index, uint16_t member_index)const
   {
      uint32_t member_data_offset;
      uint32_t num_sorted_members;
      uint32_t total_num_members;
   ...
   type_id types_manager::get_variant_case_type(type_id tid, uint16_t which)const
   { 
   ...
   uint32_t types_manager::get_variant_tag_offset(type_id tid)const
   {
      if( tid.get_type_class() != type_id::variant_or_optional_type )
         throw std::invalid_argument("Must provide a variant type to this function.");
   ...
   uint32_t types_manager::get_optional_tag_offset(type_id tid)const
   {
   ...
   types_manager::get_base_info(type_id::index_t struct_index)const
   {
      if( struct_index + 3 >= types.size() )
         throw std::out_of_range("Struct index is not valid.");
   ...
   pair<type_id, uint32_t> types_manager::get_container_element_type(type_id tid)const
   {
   ...
   type_id::index_t types_manager::add_empty_struct_to_end()
   {
      type_id::index_t index = types.size();
      types.push_back(type_id::size_align().get_storage());
      types.push_back(0);
      types.push_back(0);
      types.push_back(0);
      return index;
   ...
   void types_manager::complete_struct(type_id::index_t index, type_id::size_align sa, 
                                       uint32_t members_offset, uint16_t total_num_members, uint16_t num_sorted_members,
                                       type_id base, field_metadata::sort_order base_so)
   {   
      ...
      *itr = ( (total_num_members << 16) | num_sorted_members );
      ...
      {
         sorted_window::set(storage, (base_so != field_metadata::unsorted));
         ascending_window::set(storage, (base_so == field_metadata::ascending));
   ...
   type_id::index_t types_manager::add_array(type_id element_type, uint32_t num_elements)
   {
      type_id::index_t index = types.size();
      types.push_back(type_id::size_align().get_storage());
      types.push_back(num_elements);
      types.push_back(element_type.get_storage());
      return index;
   }
   
   type_id::index_t types_manager::add_vector(type_id element_type)
   {
      type_id::index_t index = types.size();
      types.push_back(element_type.get_storage());
      return index;
   ...
   type_id::index_t types_manager::add_optional(type_id element_type)
   {
      type_id::index_t index = types.size();
      types.push_back(type_id::size_align().get_storage());
      types.push_back(element_type.get_storage());
   ...
   type_id::index_t types_manager::add_variant(const vector<type_id>& cases)
   {
      type_id::index_t index = types.size();
      types.push_back(type_id::size_align().get_storage());
      types.push_back(static_cast<uint16_t>(cases.size()));
      for( auto t : cases )
         types.push_back(t.get_storage());
      return index;
   }

   uint8_t  types_manager::get_num_indices_in_table(type_id::index_t index)const
   {
   ...
   type_id::index_t types_manager::get_struct_index_of_table_object(type_id::index_t index)const
   {
      if( index + 3 > types.size() )
         throw std::invalid_argument("Table index is not valid.");
   ...
   types_manager::table_index types_manager::get_table_index(type_id::index_t index, uint8_t index_seq_num)const
   {
   ...
   types_manager::table_index::table_index(const types_manager& tm, type_id::index_t index, uint8_t index_seq_num)
      : tm(tm)
   {
   ...
   bool types_manager::table_index::is_unique()const
   {
      return unique_window::get(index_info);
   }

   bool types_manager::table_index::is_ascending()const
   {
      return ascending_window::get(index_info);
   }

   type_id types_manager::table_index::get_key_type()const
   {
   ...
   types_manager::table_index::get_sorted_members()const
   {
   ...
   struct compare_visitor
   {
      ...
      compare_visitor(const types_manager& tm, const raw_region& lhs, uint32_t lhs_offset, 
   ...
   int8_t types_manager::compare_object_with_key(const raw_region& object_data, const raw_region& key_data, const table_index& ti)const
   {
   ...
   int8_t types_manager::compare_objects(uint64_t lhs_id, const raw_region& lhs, uint64_t rhs_id, const raw_region& rhs, const table_index& ti)const
   {
